...
9. **Supply Chain Compromise (litellm 1.82.7)** in `corndog-suggestions/requirements.txt` — pinned to version compromised by TeamPCP CanisterWorm campaign; Datadog SCA detects the malicious package advisory. LLM Observability shows litellm.completion() call traces.
